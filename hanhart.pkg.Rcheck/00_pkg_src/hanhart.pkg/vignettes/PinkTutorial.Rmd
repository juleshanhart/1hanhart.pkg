...
example: consolidate <- function(data){
    clean <- data %>% 
        na.omit(data) %>% 
...
example: weight_in_lbs <- function(data = surveys){
  clean <- data %>% 
...
example: linearmodel <- function(data){
   clean <- data %>% 
...
example: pointplot <- function(x = weight, y = species_id, surveys){
